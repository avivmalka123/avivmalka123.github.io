...
// ── interviews calendar (password-gated, no app key: the public page holds no secrets) ──
    if(url.pathname.startsWith('/cal/')){
      ...
      if(url.pathname==='/cal/hours'&&req.method==='POST'){ const b=await req.json().catch(()=>({})); if(!/^\d{4}-\d{2}-\d{2}$/.test(b.day||'')) return json({error:'day'},400,h);
        if(!b.start_t&&!b.end_t) await sb(env,'work_hours?day=eq.'+b.day,'DELETE'); else await sb(env,'work_hours?on_conflict=day','POST',{day:b.day,start_t:b.start_t||null,end_t:b.end_t||null,updated_at:new Date().toISOString()},'resolution=merge-duplicates,return=minimal'); return json({ok:true},200,h); }
