...
export default {
  async fetch(req,env){
    ...
    try{
      ...
      if(url.pathname==='/followups'){ const rep=normName(url.searchParams.get('rep')); if(!rep) return json({error:'rep'},400,h);
        return json(await cache('followups:'+rep,600,async()=>{
