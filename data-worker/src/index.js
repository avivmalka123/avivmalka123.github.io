...
// ── interviews calendar (password-gated, no app key: the public page holds no secrets) ──
    if(url.pathname.startsWith('/cal/')){
      ...
      if(url.pathname==='/cal/list'){ const from=url.searchParams.get('from')||new Date(Date.now()-30*864e3).toISOString(); const to=url.searchParams.get('to')||new Date(Date.now()+120*864e3).toISOString();
        ...
        let hours=[]; try{ hours=await sb(env,'work_hours?select=*&day=gte.'+from.slice(0,10)+'&day=lte.'+to.slice(0,10)+'&limit=400'); }catch(e){}
        return json({role,rows,hours,pending:role==='admin'?pending:[],pendingCount:pending.length},200,h); }
      if(url.pathname==='/cal/hours'&&req.method==='POST'){ const b=await req.json().catch(()=>({})); if(!/^\d{4}-\d{2}-\d{2}$/.test(b.day||'')) return json({error:'day'},400,h);
        const cur=(await sb(env,'work_hours?select=*&day=eq.'+b.day).catch(()=>[]))[0]||{}; const row={day:b.day,start_t:b.start_t!==undefined?(b.start_t||null):(cur.start_t||null),end_t:b.end_t!==undefined?(b.end_t||null):(cur.end_t||null),updated_at:new Date().toISOString()}; if(Array.isArray(b.blocks)) row.blocks=b.blocks.filter(x=>x&&x.start_t&&x.end_t).map(x=>({start_t:String(x.start_t).slice(0,5),end_t:String(x.end_t).slice(0,5),label:String(x.label||'חסימה').slice(0,40)})); else if(cur.blocks) row.blocks=cur.blocks;
        if(!row.start_t&&!row.end_t&&!(row.blocks&&row.blocks.length)) await sb(env,'work_hours?day=eq.'+b.day,'DELETE'); else await sb(env,'work_hours?on_conflict=day','POST',row,'resolution=merge-duplicates,return=minimal'); return json({ok:true},200,h); }
      if(url.pathname==='/cal/add'&&req.method==='POST'){ const b=await req.json().catch(()=>({})); if(!b.meet_at||isNaN(new Date(b.meet_at))) return json({error:'meet_at'},400,h); const id='m_'+Date.now().toString(36)+Math.random().toString(36).slice(2,6);
        await sb(env,'interviews','POST',{id,name:b.name||'',phone:b.phone||'',rep:b.rep||'ידני',meet_at:new Date(b.meet_at).toISOString(),meet_label:b.label||'',status:'approved',source:'manual',notes:b.notes||null,decided_by:role==='admin'?'מנהל':'מראיין',decided_at:new Date().toISOString(),form:b.form||{},updated_at:new Date().toISOString()},'return=minimal'); return json({ok:true,id},200,h); }
      if(url.pathname==='/cal/outcome'&&req.method==='POST'){ const b=await req.json().catch(()=>({})); if(!b.id) return json({error:'id'},400,h); const cur=(await sb(env,'interviews?select=id,meet_at,history,outcome,outcome_reason,amount&id=eq.'+encodeURIComponent(b.id)))[0]; if(!cur) return json({error:'not found'},404,h);
        const hist=Array.isArray(cur.history)?cur.history:[]; const now=new Date().toISOString(); let patch={updated_at:now};
        if(b.outcome==='rescheduled'){ if(!b.new_meet_at||isNaN(new Date(b.new_meet_at))) return json({error:'new_meet_at'},400,h); hist.push({at:now,what:'rescheduled',from:cur.meet_at,to:new Date(b.new_meet_at).toISOString(),reason:b.reason||''}); patch={...patch,meet_at:new Date(b.new_meet_at).toISOString(),meet_label:b.label||'',outcome:null,outcome_reason:null,outcome_at:null,history:hist}; }
        else if(b.outcome==='failed'){ if(!(b.reason||'').trim()) return json({error:'reason'},400,h); hist.push({at:now,what:'failed',reason:b.reason}); patch={...patch,outcome:'failed',outcome_reason:b.reason.trim(),amount:null,outcome_at:now,history:hist}; }
        else if(b.outcome==='closed'){ const amt=Number(b.amount); if(!(amt>0)) return json({error:'amount'},400,h); hist.push({at:now,what:'closed',amount:amt,reason:b.reason||''}); patch={...patch,outcome:'closed',outcome_reason:(b.reason||'').trim()||null,amount:amt,outcome_at:now,history:hist}; }
        else if(b.outcome==='no_answer'){ const n=hist.filter(e=>e.what==='no_answer').length+1; hist.push({at:now,what:'no_answer',n}); patch={...patch,outcome:'no_answer',outcome_reason:'לא ענה ×'+n,amount:null,outcome_at:now,history:hist}; }
        else if(b.outcome==='reopen'){ hist.push({at:now,what:'reopen'}); patch={...patch,outcome:null,outcome_reason:null,amount:null,outcome_at:null,history:hist}; }
        else return json({error:'outcome'},400,h);
        await sb(env,'interviews?id=eq.'+encodeURIComponent(b.id),'PATCH',patch,'return=minimal'); return json({ok:true},200,h); }
      if(url.pathname==='/cal/edit'&&req.method==='POST'){ const b=await req.json().catch(()=>({})); if(!b.id) return json({error:'id'},400,h); const patch={updated_at:new Date().toISOString()}; if(typeof b.name==='string') patch.name=b.name.trim(); if(typeof b.phone==='string') patch.phone=b.phone.trim(); if(typeof b.notes==='string') patch.notes=b.notes.trim()||null; await sb(env,'interviews?id=eq.'+encodeURIComponent(b.id),'PATCH',patch,'return=minimal'); return json({ok:true},200,h); }
    ...
    try{
      ...
      if(url.pathname==='/active'){ const rep=normName(url.searchParams.get('rep')); let idx={}; if(sbReady(env)){ try{ for(const r of await sb(env,'tc_active?select=*&ts=gt.'+new Date(Date.now()-2*3600e3).toISOString())) idx[r.agent_key]={phone:r.phone,agent:r.agent,name:r.name,campaign:r.campaign,externalId:r.external_id||'',type:r.type,ts:r.ts}; }catch(e){} } else { const idxRaw=await env.CACHE.get('active:index'); idx=idxRaw?JSON.parse(idxRaw):{}; } const fresh=e=>e&&(Date.now()-new Date(e.ts).getTime())<2*3600e3;
        ...
        // parallel dialing: ToChat starts 2 calls at once and only the answered one ever gets a CallEnded. Return every open start
        // (candidates) so the app can resolve who is really on the line, and the last ended call for retroactive attribution.
        let candidates=[], lastEnded=null; if(sbReady(env)&&rep){ try{ const since=new Date(Date.now()-40*60000).toISOString(); const evs=await sb(env,'tc_events?select=ts,type,phone,agent,name,campaign,external_id,sid:raw->>sessionId&ts=gt.'+since+'&order=ts.asc&limit=400');
          const match=a=>{ const k=normName(a); return !!k&&(k===rep||k.includes(rep)||rep.includes(k)||k.startsWith(rep.slice(0,3))); }; const mine=evs.filter(e=>match(e.agent)&&e.phone); const isEndT=t=>/CallEnded|CallRecordCreated|AISummary|Done|Followup|SubStatus/i.test(t); const isStartT=t=>/Outgoing|Incoming|Call/i.test(t)&&!isEndT(t);
          const starts=mine.filter(e=>isStartT(e.type)), ends=mine.filter(e=>/CallEnded|CallRecordCreated|AISummary/i.test(e.type)); const endedAfter=(st)=>ends.some(e=>e.phone===st.phone&&e.ts>st.ts);
          let open=starts.filter(st=>!endedAfter(st)); open=open.filter(st=>{ const twinEnded=starts.some(o=>o!==st&&o.phone!==st.phone&&Math.abs(new Date(o.ts)-new Date(st.ts))<8000&&endedAfter(o)); return !twinEnded; });
          const seen={}; for(const st of open.slice().reverse()){ if(seen[st.phone]) continue; seen[st.phone]=1; candidates.push({phone:st.phone,name:st.name||'',campaign:st.campaign||'',externalId:st.external_id||'',ts:st.ts,sid:st.sid||'',twin:starts.some(o=>o!==st&&o.phone!==st.phone&&Math.abs(new Date(o.ts)-new Date(st.ts))<8000)}); }
          const le=ends.filter(e=>/CallEnded/i.test(e.type)).slice(-1)[0]; if(le) lastEnded={phone:le.phone,name:le.name||'',ts:le.ts}; }catch(e){} }
        return json({active:fresh(hit)?hit:null,candidates,lastEnded,agents:Object.keys(idx).filter(k=>k!=='_last'&&k!=='_')},200,{...h,'Cache-Control':'no-store'}); }
